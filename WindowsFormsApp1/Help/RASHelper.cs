...
namespace Gcashface.EDCode
{
   public class RASHelper
    ...
    {
        public static RSACryptoServiceProvider RsaFromPrivatekey(string privateKey)
        {
        ...
        public static string SignWithSHA256(string content, string privateKey)//会异常,有时正常,怀疑 HslCommunication.Core.Security.RSAHelper.GetPrivateKeyFromRSA有问题
        {
        ...
        /// <param name="privateKey">私钥</param>
        /// <returns>签名后字符串</returns>
        public static string SignWithSHA256(string content, RSACryptoServiceProvider rsa)
        {
            byte[] Data = Encoding.UTF8.GetBytes(content);
            SHA256 sh = new SHA256CryptoServiceProvider();
            byte[] signData = rsa.SignData(Data, sh);
        ...
        /// 加密
        /// </summary>
        /// <param name="DataToEncrypt">需要加密的字节</param>
        /// <param name="publicKey">公钥</param>
        /// <param name="input_charset">编码格式</param>
        /// <returns>明文</returns>
        public static string encryptData(byte[] DataToEncrypt, string publicKey, string input_charset)
        {
            string result = encrypt(DataToEncrypt, publicKey, input_charset);
            return result;
        }
        ...
        /// <param name="privateKey">私钥</param>
        /// <param name="input_charset">编码格式</param>
        /// <param name="len">秘钥位数128,256</param>
        /// <returns>明文</returns>
        public static string decryptData(string resData, string privateKey, string input_charset, int len = 128)
        {
            byte[] DataToDecrypt = Convert.FromBase64String(resData);
            string result = "";
            for (int j = 0; j < DataToDecrypt.Length / len; j++)
            {
                byte[] buf = new byte[len];
                for (int i = 0; i < len; i++)
                {
                    buf[i] = DataToDecrypt[i + len * j];
                }
